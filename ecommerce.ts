...
class ElectronicsProduct extends BaseProduct {
  getDiscountedPrice(discount: number): number {
    const maxDiscount = Math.min(discount, 20); // max 20%
    let price = super.getDiscountedPrice(maxDiscount);

    // Extra condition: 5% extra off if price > 1000
...
class ClothingProduct extends BaseProduct {
  getDiscountedPrice(discount: number): number {
    const maxDiscount = Math.min(discount, 30); // max 30%
    let price = super.getDiscountedPrice(maxDiscount);

    // Extra condition: flat ₹100 off if price > 500
...
class CreditCardPayment implements PaymentMethod {
  ...
  async processPayment(amount: number): Promise<boolean> {
    ...
    this.transactionId = "CC-" + Math.random().toString(36).substring(2, 8);
...
class PayPalPayment implements PaymentMethod {
  ...
  async processPayment(amount: number): Promise<boolean> {
    ...
    this.transactionId = "PP-" + Math.random().toString(36).substring(2, 8);
    console.log(`PayPal Payment of ₹${amount} successful`);
    return true;
  }
...
// Create products
const p1 = new ElectronicsProduct("1", "Laptop", 1500, "Electronics");
const p2 = new ClothingProduct("2", "Jacket", 800, "Clothing");
...
// Checkout
order.checkout().then(result => {
  console.log("Order Result:", result);
});
//ts-node ecommerce.ts
//tsc ecommerce.ts
//node ecommerce.js
